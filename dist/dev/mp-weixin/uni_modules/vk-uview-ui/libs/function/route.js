...
class Router {
  ...
  mixinParam(url, params) {
    ...
    let query = "";
    if (/.*\/.*\?.*=.*/.test(url)) {
      query = common_vendor.i.$u.queryParams(params, false);
      return url += "&" + query;
    } else {
      query = common_vendor.i.$u.queryParams(params);
      return url += query;
    }
  }
  // 对外的方法名称
  async route(options = {}, params = {}) {
    ...
    if (typeof options === "string") {
      ...
      mergeConfig = common_vendor.i.$u.deepClone(options, this.config);
    ...
    mergeConfig = common_vendor.i.$u.deepMerge(this.config, mergeConfig);
