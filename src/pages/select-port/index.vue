<script lang="ts" setup>
	...
	} : any = useRequest(getLocationList(searchParams), { immediate: false });
	// 航线列表
	const {
		data: areaOptions,
	...
	<view>
		...
		<view class="select-port" v-else>
			<view class="bg-neutral py-12 px-20">
				<u-search :placeholder="placeholder" shape="square" v-model="searchParams.keyword" action-text="取消"
					@change="onSearch" bg-color="#F5F7FA" @custom="cancel" />
			</view>
			<view class="mt-20 bg-neutral" v-if="type === 'POR'">
				<view v-for="(item, index) in porList" :key="index"
			...
			<view v-else-if="type === 'FND'" class="content flex mt-20 font28 font400">
				<view class="px-50 bg-neutral flex flex-column align-start">
					<view v-for="(item, index) in areaOptions" :key="index" @click="changeArea(item.id)"
						class="flex flex-column light-grey text-center font28 font400 light-grey relative mt-50" :class="[
              searchParams.routeId === item.id ? 'dull-red font-bold' : '',
            ]">
						<view class="active-bar absolute"
				...
				<view class="flex1 ml-10 font28 font-bold dull-grey bg-neutral">
					<scroll-view :scroll-y="true" style="height: 100%" :scroll-top="scrollTop">
						<view v-for="(item, index) in fndOptions" :key="index" class="flex flex-column px-32 py-40">
							<view class="mb-28">{{ item.areaInfoName }}</view>
							<view v-for="(child, childIndex) in item.portInfo" :key="childIndex"
								class="flex flex-column" @click="onClick(child)">
								<view class="font26 font400 mt-28">{{ child.cnName }}-{{ child.enName }}</view>
								<view class="font22 light-grey font400 pb-28" style="border-bottom: 1px solid #edeff2">
