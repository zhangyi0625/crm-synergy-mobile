<script lang="ts" setup>
	...
	const emit = defineEmits(["jumpEither", "refresh", "openShrink"]);
	interface PropsType {
		data : Array<any>;
		isSort : string;
		isRoute : boolean;
	...
	const refresh = (carrierCode : string) => {
		if (props.data.find((el : any) => el.searchstate)) {
			Toast('存在其他运价实时任务正在执行！')
			return
		}
	...
	<view class="freight-table mt-10">
		...
		<view v-else v-for="(item, index) in props.data" :key="index" class="mt-20">
			<view class="py-18 px-24 bg-neutral flex align-center flex-between"
				style="border-bottom: 1px solid #F5F7FA;" v-if="item.carrierCode !== ProductType.QMS && !isRoute">
				<view class="flex align-center">
				...
				<view v-if="!item.isShrink" class="flex align-center flex1 flex-end light-grey update"
					...
					<img v-if="!item.searchstate" src="/static/images/icon/refresh.png" class="w-32 h-32 ml-12"
						@click.stop="refresh(item.carrierCode)">
					<view v-else>正在更新....</view>
				</view>
			...
			<view v-if="item.products.length > 0 && !item.isShrink" v-for="(el,elIndex) in item.products" :key="elIndex"
				...
				<view class="pb-18 bg-neutral flex align-center font32 font-bold dull-grey"
					...
					{{item.products[0].por?.cnName}}{{item.products[0].por?.enName}} -
					{{item.products[0].fnd?.cnName}}{{item.products[0].fnd?.enName}}
				</view>
			...
			'.png'" class="absolute left-0 top-0 w-68 h-68" />
				...
				<view class="mt-10 flex align-center flex-between dull-grey">
					<view class="flex align-center">
						...
						<view class="px-20 schedule">船期：{{ el.cutOffDay ? el.cutOffDay : '-'}} /
