  ...
  watch: {
    defaultDate: {
      ...
      handler(val) {
        if (val) {
          if (
            this.mode == DATE_TYPES.YM &&
            val.replace(/\-/g, "/").split("/").length == 2
          ) {
            // 日期模式为年月时有可能传进来的defaultDate是2022-02这样的格式，在ios下new Date会报错，加上日期部分做兼容
            val += "-01";
          } else if (
            this.mode == DATE_TYPES.HMS ||
            this.mode == DATE_TYPES.HM
          ) {
  ...
  computed: {
    minDateObj() {
      ...
      if (minDate) {
        if (
          this.mode == DATE_TYPES.YM &&
          minDate.replace(/\-/g, "/").split("/").length == 2
        ) {
    ...
    maxDateObj() {
      ...
      if (maxDate) {
        if (
          this.mode == DATE_TYPES.YM &&
          maxDate.replace(/\-/g, "/").split("/").length == 2
        ) {
