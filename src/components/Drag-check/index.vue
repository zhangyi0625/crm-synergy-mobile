...
<template>
	<view class="custom-modal">
        ...
        props.minTitle
			...
			<view class="font30 text-center dull-grey mt-20" :class="{ 'dull-red': result == '验证失败，请重试' }">
				{{ result ? result : "" }}
			</view>
			<view class="flex align-center flex-center my-22" :style="'transform:rotate(' + deg + 'deg)'">
				<img v-if="imgUrl" :src="imgUrl" class="w-400 h-400 br-100 my-10" />
			</view>
			<view class="drag-check-item">
				<movable-area>
					<movable-view :disabled="disabled" direction="horizontal" :x="x" @change="onChange"
						@touchend="touchEnd">
		...
		<view @tap="close" @touchmove.stop.prevent :class="props.visiable ? 'pupop-model' : 'pupop-models'"></view>
	</view>
</template>
