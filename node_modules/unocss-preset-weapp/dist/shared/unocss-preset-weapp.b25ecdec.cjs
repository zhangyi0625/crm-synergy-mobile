...
const colors = require('./unocss-preset-weapp.51ea3303.cjs');
const variants$1 = require('./unocss-preset-weapp.41085966.cjs');
...
const variantColorsMediaOrClass = (options = {}) => {
  ...
  if (options?.dark === "class" || typeof options.dark === "object") {
    ...
    return [
      variants$1.variantMatcher("dark", (input) => ({ prefix: `${dark} $$ ${input.prefix}` }), transformRules),
      variants$1.variantMatcher("light", (input) => ({ prefix: `${light} $$ ${input.prefix}` }), transformRules)
    ];
  }
  return [
    variants$1.variantParentMatcher("dark", "@media (prefers-color-scheme: dark)", transformRules),
    variants$1.variantParentMatcher("light", "@media (prefers-color-scheme: light)", transformRules)
  ];
};
...
const variantImportant = {
  ...
  match(matcher, { theme }) {
    let base;
    matcher = utils.restoreSelector(matcher, theme?.transformRules);
