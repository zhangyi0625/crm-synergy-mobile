...
const variantMatcher = (name, handler, transformRules = utils.defaultRules) => {
  const re = new RegExp(`^${core.escapeRegExp(name)}[:-]`);
  return {
    name,
    match(input) {
      input = utils.restoreSelector(input, transformRules);
...
const variantParentMatcher = (name, parent, transformRules = utils.defaultRules) => {
  const re = new RegExp(`^${core.escapeRegExp(name)}[:-]`);
  return {
    name,
    match(input) {
