...
import { restoreSelector, defaultRules } from 'unplugin-transform-class/utils';
import { u as getBracket } from './unocss-preset-weapp.4031e508.mjs';

const variantMatcher = (name, handler, transformRules = defaultRules) => {
...
const variantParentMatcher = (name, parent, transformRules = defaultRules) => {
  const re = new RegExp(`^${escapeRegExp(name)}[:-]`);
  return {
    name,
    match(input) {
...
export { variantParentMatcher as a, variantGetBracket as b, variantGetParameter as c, variantMatcher as v };
