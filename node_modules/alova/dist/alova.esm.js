/**
  * alova 2.6.1 (https://alova.js.org)
...
const getLocalCacheConfigParam = (methodInstance) => {
    ...
    const getCacheExpireTs = (_localCache) => isNumber(_localCache) ? getTime() + _localCache : getTime(_localCache || undefinedValue);
 ...
 * @returns 请求状态
 */
function useHookToSendRequest(methodHandler, frontStates, useHookConfig, successHandlers, errorHandlers, completeHandlers, onRequest, sendCallingArgs = [], isFetcher = falseValue) {
    ...
    const middlewareCompletePromise = isFetcher
        ? middleware({
            ...
            fetch: (matcher, ...args) => {
                ...
                return useHookToSendRequest(methodInstance, frontStates, useHookConfig, successHandlers, errorHandlers, completeHandlers, onRequest, args, trueValue);
            },
            fetchStates,
    ...
    const responseCompletePromise = promiseCatch(promiseThen(middlewareCompletePromise, middlewareReturnedData => {
        const afterSuccess = (data) => {
            ...
            runArgsHandler(completeHandlers, completeHandlerDecorator, createAlovaEvent(2, methodInstance, sendCallingArgs, fromCache(), data, undefinedValue, 'success'));
            return data;
        };
    ...
    (error) => {
        ...
        errorLogger = isFn(errorLogger)
            ? errorLogger
            : ![falseValue, nullValue].includes(errorLogger)
                ? defaultErrorLogger
                : undefinedValue;
        ...
        !controlledLoading && (newStates.loading = falseValue);
        update(newStates, frontStates);
        runArgsHandler(errorHandlers, errorHandlerDecorator, createAlovaEvent(1, methodInstance, sendCallingArgs, fromCache(), undefinedValue, error));
        runArgsHandler(completeHandlers, completeHandlerDecorator, createAlovaEvent(2, methodInstance, sendCallingArgs, fromCache(), undefinedValue, error, 'error'));
